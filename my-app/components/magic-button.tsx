...
export default function MagicButton({ children, href, onClick, className = "", heightClass = "h-12", type = "button", disabled = false }: MagicButtonProps) {
  ...
  return (
    ...
    >
      <span
        className="absolute inset-0 pointer-events-none z-0 rounded-xl bg-[conic-gradient(from_90deg_at_50%_50%,#E2CBFF_0%,#393BB2_50%,#E2CBFF_100%)] magic-border-spin"
      />
      <span className="relative z-10 inline-flex h-full w-full cursor-pointer items-center justify-center gap-2 rounded-xl bg-background px-4 py-1 text-sm font-semibold text-foreground backdrop-blur-3xl">
        {children}
      </span>
    </Root>
  )
